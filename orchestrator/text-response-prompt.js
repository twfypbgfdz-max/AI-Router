...
// confirms which real item ID is the recommended focus - it never receives
// raw section data, never computes urgency or impact itself, and
// recommendedItemId is validated server-side to be exactly the pre-computed
// top item's real ID (or null when nothing was ranked), never a free choice.
export const SNAPSHOT_BRIEFING_INSTRUCTIONS = Object.freeze([
  ...
  "The question text lists a fixed, already-computed priority ranking, each entry showing its real itemId, in order from highest to lowest priority, or states that there are no ranked items.",
  "If at least one ranked item is listed, recommendedItemId must be exactly the itemId of the item already ranked first (position 1). Never any other item's ID, never an invented ID, never null.",
