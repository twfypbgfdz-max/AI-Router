...
// source of truth for readiness - this script adds a human-readable report
// and a start/no-start gate on top of it, nothing else. No duplicated
// readiness logic, no new reason vocabulary: every string below only
// re-describes a code checkJarvisReadiness() already produces.
//
...
export function formatReadinessReport(readiness) {
  ...
  if (readiness.state === "ready") {
    return "Jarvis core ready. Voice: bereit.";
  }
  if (readiness.state === "partial") {
    return ["Jarvis partial:", ...reasonLines].join("\n");
  }
  return ["Jarvis unavailable:", ...reasonLines].join("\n");
}
